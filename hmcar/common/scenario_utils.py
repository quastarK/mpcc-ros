...
from hmcgp.common.pytypes import VehicleState, VehiclePrediction, ParametricPose, BodyLinearVelocity, VehicleActuation
from hmcgp.common.tracks.radius_arclength_track import RadiusArclengthTrack
from hmcgp.common.tracks.track_lib import CurveTrack, StraightTrack, ChicaneTrack, DualCurveTrack, DualStraightTrack, DualChicaneTrack
from hmcgp.common.tracks.track import get_track
from hmcgp.visualization.barc_plotter_qt import BarcFigure, GlobalPlotConfigs, VehiclePlotConfigs
...
@dataclass
class DualScenarioGenParams:
    types: list = field(
        default_factory=lambda: ['Monza', 'ethz', 'ehtzMobil']) 
    egoMin: VehicleState = field(default=None)
    egoMax: VehicleState = field(default=None)
...
class DualScenarioGenerator:
    def __init__(self, genParams: DualScenarioGenParams):
    ...
    def genTrackStatic(self, trackType : str, ego_init_state : VehicleState, tar_init_state : VehicleState):
        print('Generating track, named ', trackType)
    ...
    def genTrack(self):
        ...
        Generate random scenario from ego_min, ego_max, tar_min, tar_max
        """
        TrackType = np.random.choice(self.types)
        s, x_tran, e_psi, v_long = self.randomInit(self.egoMin, self.egoMax)
        ego_init_state = VehicleState(t=0.0, p=ParametricPose(s=s, x_tran=x_tran, e_psi=e_psi), v=BodyLinearVelocity(v_long=v_long))
        ...
        while(abs(s-ego_init_state.p.s) < .5 and abs(x_tran - ego_init_state.p.x_tran) < 0.4):
            ...
            ego_init_state = VehicleState(t=0.0, p=ParametricPose(s=s, x_tran=x_tran, e_psi=e_psi), v=BodyLinearVelocity(v_long=v_long))
            s, x_tran, e_psi, v_long = self.randomInit(self.tarMin, self.tarMax)
        tar_init_state = VehicleState(t=0.0, p=ParametricPose(s=s, x_tran=x_tran, e_psi=e_psi), v=BodyLinearVelocity(v_long=v_long))

        return self.genTrackStatic(TrackType, ego_init_state, tar_init_state)


    def getLabTrack(self, ego_init_state, tar_init_state):
        track_type = self.types[0]
        track_name = track_type+'_Track'
    ...
    def genCurve(self, ego_init_state, tar_init_state):
        ...
        return DualScenarioDefinition(
            track_type='curve',
            track=DualCurveTrack(enter_straight_length=4,
                             curve_length=s,
    ...
    def genStraight(self, ego_init_state, tar_init_state):
        return DualScenarioDefinition(
            track_type='straight',
            track=DualStraightTrack(length=10, width=self.width, slack=0.8, phase_out=True),
            ego_init_state=ego_init_state,
            tar_init_state=tar_init_state,
            ego_obs_avoid_d=0.1,
            tar_obs_avoid_d=0.1
    ...
    def genChicane(self, ego_init_state, tar_init_state):
        ...
        return DualScenarioDefinition(
            track_type='chicane',
            track=DualChicaneTrack(enter_straight_length=4,
                               curve1_length=s_1,
...
class ScenarioGenerator:
    ...
    def genScenarioStatic(self, scenarioType : str, ego_init_state : VehicleState ):
        print('Generating scenario of type:', scenarioType)
        if scenarioType == 'curve':
            return self.genCurve(ego_init_state)#, tar_init_state)
        elif scenarioType == 'straight':
    ...
    def genCurve(self, ego_init_state):#, tar_init_state):
        ...
        return ScenarioDefinition(
            ...
            track=CurveTrack(enter_straight_length=4,
                            curve_length=s,
                            curve_swept_angle=alpha,
                            exit_straight_length=15,
                            width=self.width,
                            slack=0.8,
                            phase_out= False, #True, 
                            ccw=False),
            ego_init_state=ego_init_state,
            # tar_init_state=tar_init_state,
            ego_obs_avoid_d=0.1,
...
class SampleGenerator():
        ...
        for ab_p in self.abs_path:
            for filename in os.listdir(ab_p):
                if filename.endswith(".pkl"):
                    ...
                    N = scenario_data.N
                    for i in range(N-1):
                        if i%3 == 0 and scenario_data.ego_preds[i] is not None:
                            tar_st = scenario_data.ego_states[i]
                            ntar_st = scenario_data.ego_states[i+1]
                            pred_st = scenario_data.ego_preds[i]
                            dtar = tar_st.copy()
    
                            # input ???
                            tar_st.u.u_a = ntar_st.u.u_a
                            tar_st.u.u_steer = ntar_st.u.u_steer

                            # residual dynamics     
                            dtar.v.v_long = ntar_st.v.v_long - pred_st.v_long[1]
                            dtar.v.v_tran = ntar_st.v.v_tran - pred_st.v_tran[1]
                            dtar.w.w_psi = ntar_st.w.w_psi - pred_st.psidot[1]
                            
                            if elect_function(tar_st):
                                self.samples.append(Sample((tar_st), dtar, np.mean(tar_st.lookahead.curvature[:])))
                    dbfile.close()
    ...
    def useAll(self, ego_state, tar_state = None):
        return True
...
def smoothPlotResults(sim_data: SimData, speedup=1, fps=60, start_t=0, close_loop=False):
    ...
    plot_conf = GlobalPlotConfigs(buffer_length=200, draw_period=1 / (2 * fps), update_period=1 / (2 * fps),
